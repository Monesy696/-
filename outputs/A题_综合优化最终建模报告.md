...
> 文档定位：模型、代码与正式结果冻结稿。本文综合《方案二模型详解（1B-2B-3B-4B）》与《A题_方案二_最终复核报告》，并以原题、附件1、附件2和附件3中的四个结果模板为最终口径。问题一至问题四均已完成正式计算、结果导出和数值复核；正文表1至表6及图1至图7均由对应的未舍入结果对象生成。
...
| 校验项目 | 温度 | 水分浓度 | 结论 |
|---|---:|---:|---|
| 80到160区间网格最大差 | 0.000128 ℃ | 0.0000468 kg/kg | 通过阈值 |
| 160到320区间网格最大差 | 0.000202 ℃ | 0.0000116 kg/kg | 通过阈值 |
| BDF与Radau最大差 | 0.0000806 ℃ | 1.76×10^-7 kg/kg | 通过 |
| 均匀场导数最大值 | 0 | 0 | 通过 |
...
\[
t_{3,*}=206904.913\ \mathrm{s}=57.4736\ \mathrm{h}.
\tag{24a}
\]

终点处全域最大含水率位于圆柱中心。与附件3固定时间间隔对齐时，首个严格达标的60 s输出时刻为206940 s，即57.4833 h；此时未舍入的全域最大含水率为0.1499898 kg/kg。表5结果如下。
...
| 57.4736 | 0.1500 | 0.1477 | 0.1402 | 0.1249 | 0.0527 |

160、320和640个控制体所得终点分别为206974.389、206918.749和206904.913 s，相邻最细网格差为13.836 s。BDF与Radau在160网格上的终点差为0.0018 s；收紧时间容差并减小最大步长后，640网格终点变化0.0019 s；累计水分平衡相对残差为9.49×10^-5，均通过验收阈值。

为控制多方案计算量，确定性灵敏度统一使用160个控制体，其基准终点57.4929 h与640网格正式值相差69.5 s。扩散系数缩放为0.9、1.0、1.1时，终点分别为63.1549、57.4929、52.8825 h；传质系数缩放为0.9、1.0、1.1时，终点分别为58.2272、57.4929、56.9170 h。因此终点对内部扩散系数更敏感。4 h后改用附件1最后实测边界时，终点为57.1899 h，较同网格主情景提前0.3030 h，说明长期边界假设影响较小但不可完全忽略。
...
附录4物性和实测半径共同作用下，连续终点为

\[
t_{4,*}=183920.779\ \mathrm{s}=51.0891\ \mathrm{h}.
\tag{41a}
\]

此时半径为1.200 cm，全域最大含水率仍位于中心。首个严格达标的60 s输出时刻为183960 s，即51.1000 h；未舍入最大含水率为0.1499799 kg/kg。由于6 h后半径已小于1.5 cm，表6中1.5 cm和2.0 cm固定位置均不再属于药材，按模板口径留空。

| 时间/h | 0 cm | 0.5 cm | 1.0 cm | 1.5 cm | 2.0 cm | 药材表面 |
|---:|---:|---:|---:|---:|---:|---:|
...
| 36 | 0.1928 | 0.1796 | 0.1317 |  |  | 0.0560 |
| 42 | 0.1712 | 0.1604 | 0.1200 |  |  | 0.0541 |
| 48 | 0.1561 | 0.1468 | 0.1116 |  |  | 0.0530 |
| 51.0891 | 0.1500 | 0.1413 | 0.1081 |  |  | 0.0526 |

效应分解得到：附录3物性且固定半径的终点为57.4736 h，附录4物性且固定半径的终点为129.8516 h，附录4物性并加入实测收缩后的终点为51.0891 h。因此，单独更换附录4物性会使终点延后72.3781 h；在附录4物性下加入实测收缩又使终点提前78.7625 h，两者合计使Q4比Q3提前6.3845 h。该结果说明不能把Q3与Q4的总差异直接解释为单一收缩效应。

80、160和320个材料坐标控制体所得终点分别为184030.904、183942.941和183920.779 s，相邻最细网格差为22.162 s。BDF与Radau在80网格上的终点差为0.0962 s；收紧时间设置后终点变化0.0775 s；材料坐标水分平衡相对残差为2.10×10^-4。PCHIP在原始半径节点上的误差为0，且保持半径非增；恒半径退化、均匀场零导数和通量恒等检查均通过。

收缩灵敏度统一使用160个材料坐标控制体，其 \(\gamma=1.0\) 基准终点51.0953 h与320网格正式值相差22.2 s。\(\gamma=0.9,1.0,1.1\) 时，终点分别为57.0809、51.0953、45.5369 h，终点半径分别为1.280、1.200、1.120 cm；PCHIP改为线性插值时终点仅变化0.0035 h。因此，Q4结论对插值方法不敏感，但对收缩幅度较敏感。
...
下列基础工作现已全部完成，因此当前模型已形成可复现的竞赛论文基线：

- 原题四个结果文件已经完整生成；
- 网格、时间、独立算法和守恒验证已经通过；
- Q3/Q4连续终点、60 s输出终点和全域最大位置已经记录；
- Q4物性变化与几何收缩作用已经分离；
- 表1至表6、图1至图7及验证CSV均可由`main.py`重建。

在缺少新的内部观测或可靠参数范围时，不继续引入二维有限元、ALE、多相传热、神经网络、遗传算法或随机不确定性平台。新模型若依赖题目未给且文献也无法合理约束的参数，不作为正式结论。
