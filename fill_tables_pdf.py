...
OUTPUT_PDF = Path("output/pdf/A题_表1至表4已填.pdf")
...
def read_table(filename):
    ...
    if len(rows[0]) != 6:
        raise ValueError(f"{path} 应包含时间列和5个空间位置列")
...
def create_overlay(page_width, page_height, tables):
    ...
    for table in tables:
        ...
        if len(values) != len(row_centers):
            raise ValueError(f"{table['csv']} 的数据行数与题面表格不一致")
        for row_values, row_center_top in zip(values, row_centers):
            baseline_y = page_height - row_center_top - 0.34 * FONT_SIZE
            for value, center_x in zip(row_values, COLUMN_CENTERS):
